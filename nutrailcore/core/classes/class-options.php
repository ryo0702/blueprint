...
if (!class_exists('WPA_Options')) {
    class WPA_Options extends WPA_Abstract
    {
        ...
        public function add_options_html()
        {
            ...
            echo ($this->args['show_form_warning']) ? '<div class="wpa-form-result wpa-form-warning">保存されていない変更があります。</div>' : '';

            echo ($has_nav && $this->args['show_all_options']) ? '<div class="wpa-expand-all" title="全ての設定を見る"><i class="fa fa-outdent"></i></div>' : '';

            echo ($this->args['show_search']) ? '<div class="wpa-search"><input type="text" name="wpa-search" placeholder="検索" autocomplete="off" /></div>' : '';

            echo '<div class="wpa-buttons">';
            echo '<input type="submit" name="'.esc_attr($this->unique).'[_nonce][save]" class="button button-primary wpa-top-save wpa-save'.esc_attr($ajax_class).'" value="保存" data-save="保存中...">';
            echo ($this->args['show_reset']) ? '<input type="submit" name="wpa_transient[reset_section]" class="button button-secondary wpa-reset-section wpa-confirm" value="セクションをリセット"  data-confirm="本当にこのセクションをリセットしてもよろしいですか？？">' : '';
            echo ($this->args['show_reset_all']) ? '<input type="submit" name="wpa_transient[reset]" class="button button-secondary wpa-warning-primary wpa-reset-all wpa-confirm" value="全てリセット" data-confirm="本当に全てリセットしてもよろしいですか？">' : '';
            echo '</div>';
            ...
            if (!empty($this->args['show_footer'])) {
                ...
                echo '<div class="wpa-buttons">';
                echo '<input type="submit" name="wpa_transient[save]" class="button button-primary wpa-save'.esc_attr($ajax_class).'" value="保存" data-save="保存中...">';
                echo ($this->args['show_reset']) ? '<input type="submit" name="wpa_transient[reset_section]" class="button button-secondary wpa-reset-section wpa-confirm" value="セクションをリセット" data-confirm="本当にセクションをリセットしてもよろしいですか？">' : '';
                echo ($this->args['show_reset_all']) ? '<input type="submit" name="wpa_transient[reset]" class="button button-secondary wpa-warning-primary wpa-reset-all wpa-confirm" value="全てリセット" data-confirm="本当に全てリセットしてもよろしいですか？">' : '';
                echo '</div>';
