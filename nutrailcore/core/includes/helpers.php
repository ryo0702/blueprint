...
{
    $options = array();
    $path    = glob(NUTRAIL_PRESET_PATH.'/'.$name.'/*.php');
    ...
    foreach ($path as $file) {
        ...
        if (!empty($fileinfo['Name'])) {
            if ($image_select) {
                if (file_exists(NUTRAIL_PRESET_PATH.'/'.$name.'/'.$filename.'.jpg')) {
                    $options[$filename] = NUTRAIL_PRESET_URI.'/'.$name.'/'.$filename.'.jpg';
                }
            } else {
    ...
    function wpa_upload_media_options(&$result)
    {
        array_walk($result, function (&$value, $key) {
            ...
            if (!empty($value['background-image']['url'])) {
                $attachment_id  = wpa_upload_media(NUTRAIL_PRESET_URI.$value['background-image']['url']);
            ...
            if (!empty($value['id']) && !empty($value['url'])) {
                $attachment_id  = wpa_upload_media(NUTRAIL_PRESET_URI.$value['url']);
                $attachment_url = wp_get_attachment_image_url($attachment_id, 'full');
                $thumbnail_url  = wp_get_attachment_image_url($attachment_id, 'thumbnail');
 ...
 * @return string|array
 */
if (!function_exists('wpa_clean')) {
    function wpa_clean($var)
    {
        if (is_array($var)) {
            return array_map('wpa_clean', $var);
        } else {
