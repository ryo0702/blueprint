 ...
 */
function wpa_theme_default_options()
{
    $options = get_option(WPOPTIONKEY);

    if (empty($options['config-common'])) {
        $options['config-common'] = [];
    }

    if (empty($options['config-common']['colors-set'])) {
        $options['config-common']['colors-set'] = 'default';
        update_option(WPOPTIONKEY, $options);
    }
}

add_action('after_switch_theme', 'wpa_theme_default_options');
