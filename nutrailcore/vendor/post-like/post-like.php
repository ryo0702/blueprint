...
class Nutrail_Post_Like
{
    ...
    public function __construct()
    {
        ...
        /* User Profile List */
        add_shortcode('show_user_likes', array($this, 'show_user_likes'));
        /* Like Button */
        add_shortcode('likes_button', array($this, 'likes_button'));
    }

    function likes_button($atts, $content = null)
    {
        global $post;

        $post_id    = !empty($atts['id']) ? $atts['id'] : 0;
        $is_comment = !empty($atts['is_comment']) && $atts['is_comment'] == 1 ? 1 : null;
        ...
        $this->simple_likes_button($post_id, $is_comment);
    ...
    {
        wp_register_script('post-like', NUTRAIL_CORE_URL.'vendor/post-like/post-like.min.js', null, '1.0', true);
    }
    ...
    function simple_likes_button($post_id, $is_comment = null)
    {
        ...
        $text_like   = esc_html__('Thích ', 'nutrail');
        $text_unlike = esc_html__('Thích ', 'nutrail');
        ...
        $content  = sprintf('%s %s %s', $icon, $title_html, $count);
        ...
        <div class="sl-wrapper">
            <a href="<?php echo esc_url($ajax_url); ?>" class="<?php echo esc_attr(implode(' ', $class)); ?>"
               data-like="<?php echo esc_attr($text_like); ?>"
               data-unlike="<?php echo esc_attr($text_unlike); ?>">
                <?php echo apply_filters('wpa_content_like_buttom', $content, $icon, $count, $title_html, $text_like, $text_unlike); ?>
            </a>
        </div>
        <?php
    } // wpa_simple_likes_button()
    ...
    function get_liked_icon()
    {
        ...
        $icon = '<span class="e-icon-thumbs-up"></span>'; // ❤, 💗

        return apply_filters('wpa_filter_like_icon', $icon);
    } // get_liked_icon()
    ...
    function get_unliked_icon()
    {
        ...
        return apply_filters('wpa_filter_unlike_icon', $icon);
    } // get_unliked_icon()
    ...
    function show_user_likes($user_ID = null)
    {
        ...
        <table class="form-table">
            <thead>
            <tr>
                <th><label for="user_likes"><?php _e('You Liked:', 'nutrail'); ?></label></th>
            </tr>
            </thead>
            <tbody>
            <?php
            ...
            if ($like_query->have_posts()) :
                ...
                <tr>
                    <td><?php _e('You do not like anything yet.', 'nutrail'); ?></td>
                </tr>
            <?php endif;
            wp_reset_postdata();
            ?>
    ...
    <?php } // wpa_show_user_likes()
}

return new Nutrail_Post_Like();