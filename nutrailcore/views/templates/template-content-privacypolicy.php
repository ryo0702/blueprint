...
$thiskey = 'config-privacypolicy';
$childkey = 'privacypolicy';
...
<div class="fmb-m">
    <p>
    <?php echo @$this_post_meta[$childkey.'_comapny']; ?>（以下、「当社」といいます。）は、本ウェブサイト上で提供するサービス（以下,「本サービス」といいます。）における、ユーザーの個人情報の取扱いについて、以下のとおりプライバシーポリシー（以下、「本ポリシー」といいます。）を定めます。
    </p>
</div>
<div class="fmb-m">
    <h3>第1条（個人情報）</h3>
...
<div class="fmb-m">
    ...
    <ul class="list">
        ...
        <?php
        if(!empty(@$this_post_meta[$childkey.'_content_addsense'])){
            echo '<li>Cookieを使用して配信する情報を決定するため(パーソナライズ広告はユーザー側で無効にすることも可能です)。</li>';
        }
        if(!empty(@$this_post_meta[$childkey.'_content_ga'])){
            echo '<li>ユーザーのアクセス動向を計測するためGoogleAnalyticsを使用するため。</li>';
        }
        if(!empty(@$this_post_meta[$childkey.'_content_contactform'])){
            echo '<li>お問合せフォームでのご連絡のため。</li>';
        }
        ?>
...
<div class="fmb-m">
    ...
    <ul>
    <?php
    if(!empty(@$this_post_meta[$childkey.'_comapny'])){
        echo '<li>社名：'.@$this_post_meta[$childkey.'_comapny'].'</li>';
    }
    if(!empty(@$this_post_meta[$childkey.'_people'])){}
    elseif(!empty(@$this_post_meta[$childkey.'_people'])){
        echo '<li>責任者：'.@$this_post_meta[$childkey.'_people'].'</li>';
    }
    if(!empty(@$this_post_meta[$childkey.'_address'])){
        echo '<li>所在地：'.@$this_post_meta[$childkey.'_address'].'</li>';
    }
    if(!empty(@$this_post_meta[$childkey.'_link']['text'])){
        echo '<li>お問合せ先：<a href="'.@$this_post_meta[$childkey.'_link']['url'].'">'.@$this_post_meta[$childkey.'_link']['text'].'</a></li>';
    }
    ?>
    </ul>
</div>
